...
uint16_t instrBufferFillCount = 0;
...
uint8_t logo_inject_instructions = LOGO_INJECT_BUFFER_EMPTY ;
...
//		break;
	...
	currentTurtle = LOGO_TURTLE_PLANE ;
	penState = 0 ; // 0 means down.  more than 0 means up
	
	turtleLocations[LOGO_TURTLE_PLANE].x._.W1 = GPSlocation.x ;
	turtleLocations[LOGO_TURTLE_PLANE].y._.W1 = GPSlocation.y ;
	turtleLocations[LOGO_TURTLE_PLANE].z = GPSlocation.z ;
	
	turtleLocations[LOGO_TURTLE_CAMERA].x._.W1 = GPSlocation.x ;
	turtleLocations[LOGO_TURTLE_CAMERA].y._.W1 = GPSlocation.y ;
	turtleLocations[LOGO_TURTLE_CAMERA].z = GPSlocation.z ;
	...
	turtleAngles[LOGO_TURTLE_PLANE] = angle ;
        turtleAngles[LOGO_TURTLE_CAMERA] = angle ;
...
void update_goal_from( struct relative3D old_goal )
{
	...
	lastGoal.x = new_goal.x = (turtleLocations[LOGO_TURTLE_PLANE].x._.W1) ;
	lastGoal.y = new_goal.y = (turtleLocations[LOGO_TURTLE_PLANE].y._.W1) ;
	lastGoal.z = new_goal.z = turtleLocations[LOGO_TURTLE_PLANE].z ;
	...
	new_goal.x = (turtleLocations[LOGO_TURTLE_CAMERA].x._.W1) ;
	new_goal.y = (turtleLocations[LOGO_TURTLE_CAMERA].y._.W1) ;
	new_goal.z = turtleLocations[LOGO_TURTLE_CAMERA].z ;
...
int16_t logo_value_for_identifier(char ident)
{
	...
	switch (ident) {
		case LOGO_VAL_DIST_TO_HOME: // in m
			return sqrt_long(IMUlocationx._.W1 * (int32_t)IMUlocationx._.W1 + IMUlocationy._.W1 * (int32_t)IMUlocationy._.W1) ;

		case LOGO_VAL_DIST_TO_GOAL: // in m
			return tofinish_line ;

		case LOGO_VAL_ALT: // in m
			return IMUlocationz._.W1 ;

		case LOGO_VAL_CURRENT_ANGLE: // in degrees. 0-359 (clockwise, 0=North)
			return get_current_angle() ;

		case LOGO_VAL_ANGLE_TO_HOME: // in degrees. 0-359 (clockwise, 0=North)
			return get_angle_to_point(0, 0) ;

		case LOGO_VAL_ANGLE_TO_GOAL: // in degrees. 0-359 (clockwise, 0=North)
			return get_angle_to_point(IMUlocationx._.W1, IMUlocationy._.W1) ;

		case LOGO_VAL_REL_ANGLE_TO_HOME: // in degrees. -180-179 (0=heading directly towards home. clockwise offset is positive)
		{
		...
		case LOGO_VAL_REL_ANGLE_TO_GOAL: // in degrees. -180-179 (0=heading directly towards goal. clockwise offset is positive)
		{
...
boolean process_one_instruction( struct logoInstructionDef instr )
{
	...
	switch (instr.cmd)
	{
		...
		case LOGO_CMD_SET_TURTLE: // Set Turtle (choose plane or camera target)
			currentTurtle = (instr.arg == LOGO_TURTLE_CAMERA) ? LOGO_TURTLE_CAMERA : LOGO_TURTLE_PLANE ;
			break ;
...
void process_instructions( void )
{
	...
	while (1)
	{
		...
		if ( do_fly && penState == 0 && currentTurtle == LOGO_TURTLE_PLANE )
			break ;
